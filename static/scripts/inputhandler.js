...
function draw_chart(city, data_label, xaxis_data, chart_data) {
  ...
  // set dimensions so it's not a 0-pixel element
  newElem.width = 800;
  newElem.height = 400;
  newElem.style.marginBottom = "20px";

  var elemId = city + "-" + data_label;
  ...
  new Chart(newElem, {
    ...
    options: {
      ...
      scales: {
        y: { beginAtZero: false }, // weather not at zero
...
function get_city_weather_graphs(city) {
     ...
     for (const [key, value] of Object.entries(fieldData)) {
        ...
        for (var i = 0; i < vals.length; i++) {
            ...
            // Ignore entries that are "-9999"
            if (val !== "-9999" && val !== "") {
                // Convert to decimal (e.g., 411 becomes 41.1)
                var numericVal = parseFloat(val) / 10.0;
                values.push(numericVal);

                // Set the day for the x-axis
        ...
        // Only draw if we actually processed valid numbers
        if (values.length > 0) {
